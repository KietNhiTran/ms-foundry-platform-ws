...
> **⚠️ The `ref/` folder if it exists, it contains content from a previous workshop iteration. It is NOT part of the current workshop and must NOT be used directly.**

Rules for handling `ref/` content:

1. **Never copy content verbatim** from `ref/` into new modules or `shared/docs/`.
2. **Treat as inspiration only** — if a topic in `ref/` is relevant to a new module, use it as a starting point but:
   ...
   - Rewrite to fit the target module's structure and audience
